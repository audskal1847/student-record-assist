...
with st.sidebar:
    ...
    # 🔥 에러 유발 모델 삭제 후 최신 초고성능 플래그십 Gemini 3.5 Pro 탑재 완료
    model_options = {
    ...
    st.caption("*(참고: 구글 API 키 입력 시에는 자동으로 gemini-1.5-pro 네이티브 모델로 빌드됩니다)*")
    st.divider()
...
with st.expander("📁 엑셀 파일 업로드 및 전체 학생 일괄 자동 생성 패널 (클릭)", expanded=False):
    ...
    # 🔥 기능 2: 전교생 일괄 생성 및 다운로드 엔진 (독서 연계 완전 강화)
    st.markdown("---")
    ...
    if st.button("🔥 엑셀 내 모든 학생 일괄 자동 완성 시작", type="secondary", use_container_width=True):
        if not api_key: st.error("API 키를 먼저 입력해 주세요!")
            ...
            for index, row in df_students.iterrows():
                ...
                b_acts = []
                # 대괄호 완전히 없애고 원문 서사 구조에 직접 명사 결합하도록 프롬프트 분사 체계 제어
                ...
                🚨 [서술 불변의 4대 철칙] 🚨
                1. 활동 기호 금지: 활동명과 도서명 양옆에 대괄호([ ]), 콜론(:), 대시(-) 등의 특수문자를 절대로 쓰지 마세요. 오직 작은따옴표('')만 사용해야 합니다. (예: '도시 성격 유형 검사 프로젝트' 활동에서~)
                2. 서술 분량 1:1:1 절대 균형: 제공된 '활동명1', '활동명2', 그리고 '도서명'의 탐구 묘사 분량과 중요도를 균등하게 33%씩 배분하세요. 어느 한쪽이 요약되거나 끊기지 않게 꽉 채워 이어 쓰세요.
                3. 고유 명칭 누락 차단: 제공된 고유 명칭인 '{b_a1n}', '{b_a2n}', '{b_bk}'는 절대로 생략하지 말고, 반드시 작은따옴표 안에 감싸 문맥에 고스란히 노출하세요.
                4. 독서 연계 종결: 마지막 문장 또는 결론 직전 단락은 무조건 책 '{b_bk}'를 읽고 파생된 질문에 대답하거나 심화 탐구한 과정으로 장식되어야 합니다. 문장이 중간에 흐지부지 잘리는 일은 절대 없어야 합니다.

                - 어조: 무조건 '~함', '~임', '~됨' 개조식 명사형 종결어미. 주어('학생은') 제외.
                ...
                [독서 심화 활동 데이터] (🚨반드시 문장 속에 끊김 없이 유기적으로 완성할 것)
                {b_book_str}
                
                → 마크다운 기호 없이, 줄바꿈이 아예 없는 완벽한 한 단락의 교과세특 결과물만 바로 출력하세요."""
                ...
                try:
                    ...
                    if b_cb < target_min:
                        b_adj = f"문장 분량이 많이 부족합니다. 제공된 모든 활동명과 도서명을 작은따옴표('') 안에 명시하면서 탐구 내용을 1:1 비율로 풍성하게 확장하여 정확히 1450바이트로 교정하세요.\n\n[원본 데이터]\n{b_acts}\n{b_book_str}\n\n[현재 문장]\n{b_res}"
...
if submit:
    ...
    for i in range(1, 3):
        if st.session_state[f'a{i}n'].strip() and st.session_state[f'a{i}d'].strip():
            # 대괄호 기호를 원천 차단하고 프롬프트 전달 설계로 완전 교정
    ...
    if not api_key: st.error("API 키를 입력해 주세요!")
        ...
        try:
            ...
            🚨 [서술 불변의 4대 철칙 - 오차 없이 엄수] 🚨
            1. 대괄호 기호 완전 금지: 결과물 문장 어디에도 대괄호([ ])나 콜론(:) 기호가 노출되어서는 안 됩니다. 명칭들은 반드시 오직 작은따옴표('') 기호로만 감싸 표기하세요.
            2. 1:1:1 완벽 서술 밸런스: 입력된 모든 활동 내용들과 '도서명' 연계 묘사의 글자 수 비율을 정확히 동등하게 삼등분하여 균형을 맞추세요. 뒤쪽 데이터라고 요약하거나 흘려보내지 마세요.
            3. 활동명/도서명 명시 필수: 데이터에 제공된 이름들을 임의로 누락하지 말고 문장 속에 자연스럽게 작은따옴표 안에 녹여내어 서사적 개연성을 구축하세요.
            4. 독서 서사 완성형 매듭: 독서 심화 데이터가 입력되었을 경우, 문장의 최종반부는 반드시 책 내용을 기반으로 발전시킨 심화 탐구 행동으로 구성하여 인과관계를 완성하며 끝나야 합니다. 문장이 툭 끊기는 현상을 절대 방지하세요.
            
            - 무조건 '~함', '~임' 명사형 종결어미 사용. 주어 생략. 한 단락 작성.
            - 분량: 공백 포함 480~500자 (1420~1470바이트) 영역 절대 사수.
            ...
            → 기호 오작동 없는 완벽한 흐름의 한 단락 본문만을 즉시 출력하세요."""
            ...
            for attempt in range(2):
                ...
                if cb > target_max:
                    adj_prompt = f"아래 문장 분량이 초과되었습니다. 고유 명칭의 작은따옴표('') 표기와 독서 마무리 서사 구조를 그대로 유지하면서 정확히 1450바이트로 압축 정리하세요.\n\n[문장]\n{result}"
                else:
                    adj_prompt = f"아래 문장 분량이 부족합니다. 입력된 모든 활동명과 도서명의 서사 비중을 1:1로 맞추고, 독서 연계 파트의 구체적 탐구 과정을 덧붙여서 1450바이트로 알차게 확장하세요.\n\n[원본 백업]\n{activities_str}\n{book_data_str}\n\n[현재 문장]\n{result}"
                try:
