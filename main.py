...
CURRICULUM_DATA_2022 = {
    ...
    "기타(생활/교양/예체능)": {
        "기술·가정": ["인간 발달과 가족", "가정생활 and 안전", "기술 시스템", "기술 활용"],
...
# 🔥 구글/오픈라우터 자동 감지 통합 통신 함수 (선택 모델 및 고정 기능 연동)
    ...
    if api_key.startswith("sk-or-"):
        client = OpenAI(
            ...
            default_headers={
                ...
                "X-Title": "Student Record Assist",
            }
        )
        response = client.chat.completions.create(
            model=selected_model, # 오픈라우터는 지정된 선택 모델 사용
            max_tokens=2000, 
        ...
        # 🔥 선생님의 지시대로 구글 라이브러리는 gemini-3.5-flash로 완전히 고정
...
with st.sidebar:
    ...
    st.caption("*(참고: 구글 API 키 입력 시에는 자동으로 gemini-3.5-flash 모델로 작동합니다)*")
    st.divider()
...
st.caption("진행한 활동의 개수만큼 입력하세요. ❗️모든 활동 데이터(특히 활동 2 포함)의 상세한 탐구 묘사는 절대 누락되거나 축약되지 않고 100% 반영됩니다.")
...
with col_act1:
    ...
    act1_desc = st.text_area("활동 1 상세 내용", placeholder="구체적인 역할, 성취 등을 입력하세요.", height=100)
...
with col_act2:
    ...
    act2_desc = st.text_area("활동 2 상세 내용", placeholder="여기에 입력된 차별화된 디테일이 생기부 출력값에 확실히 반영됩니다.", height=100)
...
if submit:
    ...
    if not api_key: st.error("API 키를 입력해 주세요!")
        ...
        try:
            ...
            # 🔥 고도화된 프롬프트 (내용 누락 원천 차단 및 정밀 분량 가이드)
            ...
            2. 전개부 (모든 활동의 구체적 반영 및 유기적 연계):
               - 🚨 [가장 중요] 입력된 {num_activities}개의 활동은 단 하나도 누락하거나 흐리게 압축해선 안 됩니다! 특히 활동 1과 활동 2 모두 원문에 기재된 학생만의 차별화된 세부 탐구 묘사, 구체적인 행동, 성취를 세밀하게 풀어써야 합니다.
            ...
            [활동 내역 및 세부 내용] (총 {num_activities}개 - 활동 2를 포함하여 전체 묘사 모두 반영할 것)
            {activities_str}
            ...
            # 🔥 2단계 정밀 분량 및 데이터 검증 조절 루프 (선택된 모델 유지)
            for attempt in range(2):
                ...
                if cb > target_max:
                    ...
                    adj_prompt = f"""아래 원본 문장의 분량이 한도 영역을 초과했습니다. 한글 공백 포함 {target_chars}자 (약 1450바이트) 범위로 안전하게 압축하세요.
                    🚨 [압축 시 절대 규칙]
                    1. 뼈대 유지: 입력된 모든 고유 활동 내용과 3단계 인과 구조는 그대로 살리되, 불필요한 반복 어휘나 만연체 수식어만 간결하게 가다듬어 바이트를 조절하세요.
                    2. 모든 문장 끝은 무조건 '~함', '~임' 명사형 종결어미 유지.
                    3. 작은따옴표('')로 묶인 고유 활동명 절대 보존.
                    ...
                    adj_prompt = f"""아래 원본 문장의 분량이 너무 짧습니다. 한글 공백 포함 {target_chars}자 (약 1450바이트) 조건에 정확히 도달할 수 있도록 내용을 풍성하게 보강하세요.
                    🚨 [확장 시 절대 규칙]
                    1. 디테일 보완: 아래 제공된 [원본 데이터 백업] 내용(특히 활동 2의 세부 묘사 등)을 다시 한번 꼼꼼하게 읽고, 학생이 수행한 구체적인 행동, 적용 기법, 구체적인 탐구 성과를 상세하게 문장 사이에 추가하여 길이를 늘리세요.
                    2. 모든 문장 끝은 무조건 '~함', '~임' 명사형 종결어미 유지.
                    3. 작은따옴표('')로 묶인 고유 활동명 절대 보존.
            ...
            if remaining_numbers:
                st.warning(f"⚠️ 시스템이 수치를 정성적으로 치환했으나, 일부 숫자 표현이 남아있을 수 있습니다: {', '.join(remaining_numbers[:5])}")
            else:
                st.info(f"✅ 모든 활동 디테일이 보존되었으며, 선택하신 AI 모델로 3단계 연계가 완료되었습니다!")
                
        except Exception as e:
            box.error(f"오류가 발생했습니다: {e}")
            st.info("💡 입력하신 API 키의 형태가 선택하신 모델 플랫폼과 일치하는지, 혹은 충전 잔액이 충분한지 확인해주세요.")

# ===== 8. 푸터 =====
st.divider()
st.markdown("""
<div style='text-align: center; color: #666; padding: 20px; font-size: 15px;'>
    🏫 <b>학생부 입력 어시스트 시스템 v5.5 (Multi-Model 통합본)</b><br>
    만든이: 신선여자고등학교 김명남<br>
</div>
