...
if submit:
    ...
    if not api_key: st.error("API 키를 입력해 주세요!")
        ...
        try:
            ...
            🚨 [작성 및 배치 절대 규칙 - 매우 중요!!!] 🚨
            1. 첫 문장 고정 및 유기적 연계: 
               - [교과 핵심 키워드] 및 [{curriculum_version} 핵심 역량]이 제공된 경우, 이 개념을 바탕으로 학생이 어떤 역량을 갖추었는지 평가하는 문장으로 전체 글의 시작을 여세요. 
               - ❌ 단순한 개념 설명이나 사전적 정의로 끝내지 마세요!
               - ✅ 반드시 "이러한 교과 핵심 개념(또는 역량)을 바탕으로 ~한 능력을 탁월하게 보여줌(발휘함)"과 같이 서술하세요.
               - 🚨 [핵심] 첫 문장에서 제시한 '핵심 역량'과 뒤이어 나오는 '[활동명]' 및 그 상세 내용이 억지로 붙인 것처럼 보이지 않도록 **논리적이고 인과적으로 완벽하게 연결**하세요.
            2. 활동명 원문 보존: 제공된 [활동 내역]의 '활동명'은 절대 임의로 바꾸거나 생략하지 마세요! 생성되는 문장 내에 작은따옴표('')로 묶어 원문 그대로 출력하세요.
            3. 문장 길이와 호흡: 문장이 너무 짧은 단답형으로 툭툭 끊어지지 않도록 주의하세요. 적절한 연결 어미를 사용하여 호흡이 길고 매끄러운 문장으로 구성하세요.

            🚨 [필수 분량] 한글 정확히 {target_chars}자(±8자) / 1420~1470바이트 / 한 단락(줄바꿈 절대 금지)!

            🚨 [구체적 숫자 절대 금지!]
            검증되지 않은 수치(예: "주민 7명", "3차례 회의") 절대 금지! ✅ 대신 "여러 명의", "다수의", "수차례" 등 정성적 표현 사용.

            🚨 [기타 절대 금지]
            1. 마크다운(별표, #, - 등) 사용 금지  
            2. '학생은/학생이/해당 학생에게서' 등 주어 표현 금지
            ...
            if remaining_numbers:
                ...
                st.info(f"✅ {curriculum_version}의 핵심 역량과 활동 내역이 완벽하게 연계된 개조식 문장이 작성되었습니다!")
